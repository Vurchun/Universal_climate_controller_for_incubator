...
#define DHTTYPE DHT11                                                 // выбор типа датчика DHT 11 
...
int TimeFanWork = 30;                                                 // время проветривания инкубатора от СО2 в секундах
...
//  RTC.stopClock();
//  RTC.fillByYMD ( 2017 , 4 , 6 );
//  RTC.fillByHMS ( 21 , 37 , 0 );
//  RTC.setTime();
//  RTC.startClock();
  ...
  lcd.setCursor(0, 1);  lcd.print("By_Vurchun_V-0");
  delay (3000);
  lcd.clear();
  dht.begin();
  currentTime = millis();
...
void PressKeyMenu ()                                                                      // Вычиление нажатия кнопок  
{
 ...
 //Serial.print( "                              Resistant key button module=" ); Serial.print( buttons_Menu ); Serial.println( " " );
  if ( buttons_Menu < 10 ) {PressingButtons = 1;}                                  // 1       
    else  if ( buttons_Menu > 60  && buttons_Menu < 75  ) PressingButtons = 2;     // 4       
      else  if ( buttons_Menu > 80 && buttons_Menu < 100 ) PressingButtons = 3;     // 2      
        else  if ( buttons_Menu > 175 && buttons_Menu < 187 ) PressingButtons = 4;  // 3      
          else  if ( buttons_Menu > 195 && buttons_Menu < 215 ) PressingButtons = 5; // 5       
