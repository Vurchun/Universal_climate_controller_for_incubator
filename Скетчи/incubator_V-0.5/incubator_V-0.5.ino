...
#define Bmenu   0xFF9867
#define Bup     0xFF18E7
#define Bdown   0xFF4AB5
#define Brotate 0xFF629D
#define Benter  0xFF38C7  
...
const int OutputPin[6] = { 3, 4, 5, 6, 8, 9};                           // возможные варианты выбора выходов
int tf = 0;
...
void setup()
{
  ...
  Serial.println("Enabled IRin");// Определяем пин подключения питания датчика температуры  
  //RTC.stopClock();
 ...
 //RTC.setTime();
  ...
  lcd.setCursor(0, 1);  lcd.print("By_Vurchun_V-0.5");
  delay(3000);
  lcd.clear();
  dht.begin();
  currentTime = millis();
...
void PressKeyMenu()                                                                      // Вычиление нажатия кнопок  
{
  ...
  if (buttons_Menu > 0 && buttons_Menu < 20) { PressingButtons = 1; }           // меню       
     ...
     if(results.value == Bmenu  ) PressingButtons = 5 ;     // 5 переворот 
     if(results.value == Bup    ) PressingButtons = 2 ;     //2  вверх 
     if(results.value == Bdown  ) PressingButtons = 3 ;     // 3 вниз  
     if(results.value == Benter ) PressingButtons = 4 ;     // 4 выбор 
     if(results.value == Bmenu  ) PressingButtons = 1 ;     // 1 меню 
     else   PressingButtons = 0; 
...
void loop()
{
  ...
  case 53: {  lcd.clear(); lcd.setCursor(0, 0); lcd.print(F("PinRot_right"     )); lcd.setCursor(0, 1); lcd.print("in relay No="); int DispRelay = PinRot_right - 1; if (DispRelay < 1) lcd.print("OFF"); else  lcd.print(DispRelay);  PrintMenuWrite(FlagMenu); delay(100);                           break; }
  case 54: {  lcd.clear(); lcd.setCursor(0, 0); lcd.print(F("PinRot_left"      )); lcd.setCursor(0, 1); lcd.print("in relay No="); int DispRelay = PinRot_left - 1; if (DispRelay < 1) lcd.print("OFF"); else  lcd.print(DispRelay);  PrintMenuWrite(FlagMenu); delay(100);                           break; }
