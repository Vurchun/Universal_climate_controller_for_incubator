...
const int OutputPin[6] = { 2, 3, 4, 5, 7, 8};                           // возможные варианты выбора выходов
int tf = 0;
...
uint8_t temp_del[8]           = { B00000, B00000, B00000, B00000, B00100, B01010, B11111 };     // закодирована в двоичной системе ДЕЛЬТА
uint8_t   Hot_ON[8]           = { B01111, B10000, B01110, B00001, B01110, B10000, B01111 };     // закодирована в двоичной системе ОБОГРЕВ
uint8_t   Fan_ON[8]           = { B00000, B11011, B11011, B00100, B00100, B11011, B11011 };     // закодирована в двоичной системе ОБДУВ
...
int PinFan = 4;                             // первоначально обдув не подключен ( подключен к отключенному 0-му выходу)
int PinRot_right = 7;                       // первоначальное подключение двигателя переворота яиц на пин №8
int PinRot_left = 8;                        // первоначальное подключение двигателя переворота яиц на пин №7

int MotorSpeed = 125;
int MoveTime   = 1250;
L298N motor(20,20,20,PinRot_left,PinRot_right,6);
...
void setup()
{
  ...
  //RTC.stopClock();
 // RTC.fillByYMD(2018, 6, 12);
 //RTC.fillByHMS(15, 23, 0);
 //RTC.setTime();
  //RTC.startClock();
  ...
  lcd.setCursor(0, 1);  lcd.print("By_Vurchun_V-0.3");
  delay(3000);
  lcd.clear();
  dht.begin();
  currentTime = millis();
...
void StartFan() // включение и отключение вентилятора и печать их состояний на дисплей 
{
  ...
  if (FlagTempFan == 1 || FlagTimeFan == 1) {
    Serial.print("PinFan - "); Serial.print(PinFan);Serial.println(" ");
    digitalWrite(PinFan, LOW); lcd.setCursor(13, 1); lcd.print("\6"); 
    }
...
void StartHot()                                                     // включение и отключение обогрева и печать их состояний на дисплей
{
  ...
  {
    Serial.print("              PinHot - "); Serial.print(PinHot);Serial.println(" ");
...
void StartHum()
{
  ...
  {
    Serial.print("                        PinHum - "); Serial.print(PinHum);Serial.println(" ");
...
void PressKeyMenu()                                                                      // Вычиление нажатия кнопок  
{
  ...
  else  if (buttons_Menu > 45 && buttons_Menu < 70) PressingButtons = 3;     // вниз      
  else  if ( buttons_Menu > 230 && buttons_Menu < 240)PressingButtons = 4;      // выбор      
  else  if (buttons_Menu > 200 && buttons_Menu < 225) PressingButtons = 5;  // переворот       
...
void loop()
{
  ...
  if (currentTime >= (loopTime))
  {
                    ...
                    if (MainMenu == 3 && FlagMenu == 0 && SubMenu == 1) { bank++; StartMillis = currentMillis;  delay(200); if (bank > 4) bank = 4; }
                      ...
                      if (MainMenu == 4 && FlagMenu == 1) {
                        ...
                        if (MainMenu == 5 && FlagMenu == 1) {
                          switch (SubMenu) {
                          case 1: {  k++; if (k > 4) k = 4; PinHot = OutputPin[k]; StartMillis = currentMillis; delay(200); EEPROM.write(100, PinHot); delay(100); break; }
    ...
    case 1:      // главное меню "1"
      ...
      case 9:  m = 19; break;
      } // вывод на экран главного меню "1" подменю "9"
      break;
    ...
    case 5:
      ...
      case 5:  m = 55; break;
      } // вывод на экран главного меню "5" подменю "5"
      break;
    case 6:
      switch (SubMenu) {
  ...
  case 51: {  lcd.clear(); lcd.setCursor(0, 0); lcd.print(F(" hotting  swith ")); lcd.setCursor(0, 1); lcd.print("in relay No="); int DispRelay = PinHot ;        lcd.print(DispRelay);  PrintMenuWrite(FlagMenu); delay(100);                           break; }
  case 52: {  lcd.clear(); lcd.setCursor(0, 0); lcd.print(F(" colling  swith ")); lcd.setCursor(0, 1); lcd.print("in relay No="); int DispRelay = PinFan ;         lcd.print(DispRelay);  PrintMenuWrite(FlagMenu); delay(100);                           break; }
  case 53: {  lcd.clear(); lcd.setCursor(0, 0); lcd.print(F("PinRot_right"     )); lcd.setCursor(0, 1); lcd.print("in relay No="); int DispRelay = PinRot_right ;  lcd.print(DispRelay);  PrintMenuWrite(FlagMenu); delay(100);                           break; }
  case 54: {  lcd.clear(); lcd.setCursor(0, 0); lcd.print(F("PinRot_left"      )); lcd.setCursor(0, 1); lcd.print("in relay No="); int DispRelay = PinRot_left ;   lcd.print(DispRelay);  PrintMenuWrite(FlagMenu); delay(100);                           break; }
  case 55: {  lcd.clear(); lcd.setCursor(0, 0); lcd.print(F(" humiditi swith ")); lcd.setCursor(0, 1); lcd.print("in relay No="); int DispRelay = PinHum ;         lcd.print(DispRelay);  PrintMenuWrite(FlagMenu); delay(100);                           break; }
