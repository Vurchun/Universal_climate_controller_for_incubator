...
DallasTemperature sensors(&oneWire);                                               // инициализация DS18b20 на 13 входе   
  ...
  IRrecv pult(9);                              //инициалезация пульта ( ик датчика )
...
uint8_t Hot_ON[8] = { B01111, B10000, B001110, B00001, B01110, B10000, B01111 };     // закодирована в двоичной системе ОБОГРЕВ
...
label_1:
  ...
  if (buttons_Limit_Swith == 1)
  {
    ...
    if (Rotate == 1) {
      lcd.setCursor(0, 1);  lcd.print("start rotate EGG");  digitalWrite(PinRot_right, HIGH); digitalWrite(PinRot_left, LOW);
    }
    else if (Rotate == 0) {
      lcd.setCursor(0, 1);  lcd.print("start rotate EGG");  digitalWrite(PinRot_left, HIGH); digitalWrite(PinRot_right, LOW);
    }goto label_1;
  }
label_2:
...
void PressKeyMenu()                                                                      // Вычиление нажатия кнопок  
{
  ...
  else  if ( buttons_Menu > 235 && buttons_Menu < 245)PressingButtons = 4;      // выбор      
