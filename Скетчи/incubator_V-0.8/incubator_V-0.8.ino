...
PID myPID(&Tnow, &Output, &Setpoint, consKp, consKi, consKd, DIRECT);  //Specify the links and initial tuning parameters
...
int DDHI,IDHI;
int language = 3;
#define MaximumDecreaseDayHumiditiIncubation 365  
#define MinimumDecreaseDayHumiditiIncubation 1 
#define MaximumIncreaseDayHumiditiIncubation 365  
...
int PinFan = 3;                             // первоначально обдув не подключен ( подключен к отключенному 0-му выходу)
int PinHum = 4;                             // первоначальное подключение увлажнителя на реле №2
int PinLite = 5;                             // первоначальное подключение Света на реле №2
...
void setup()
{
 ...
 myPID.SetMode(AUTOMATIC);
  ...
  PinHot = EEPROM.read(100); delay(100);        // чтение из ячейки памяти "100" EEPROM настройки пина обогрева
  ...
  KHumiditiIncubation = EEPROM.read(14); delay(100);     
  DecreaseDayHumiditiIncubation = EEPROM.read(15); delay(100);
  IncreaseDayHumiditiIncubation = EEPROM.read(16); delay(100); 
  language = EEPROM.read(17); delay(100);                                       
  ...
  lcd.setCursor(0, 1);  lcd.print("By_Vurchun_V-0.8");
  delay(3000);
  lcd.clear();
  dht.begin();
  currentTime = millis();
...
void SaveToEEPROM(int BankSave)                                                         // запись данных во внутренний EEPROM 
{
  ...
  BS = BankSave * 20 + 17;  EEPROM.write(BS,language); delay(100);  // запись Языка в ячейку "16" банка "bank"
...
void LoadFromEEPROM(int BankLoad)                                                       // загрузка данных из внутреннего EEPROM
{
  ...
  BL = BankLoad * 20 + 17;  language  = EEPROM.read(BL); delay(100);// чтение  Языка в ячейку "16" банка "bank"
  TimeFaningInterval = TimeFanWork * 1000;                 //длительность работы вентилятора при продувке инкубатора от СО2 
  TimeIntervalFaningInterval = TimeIntervalFanWork * 60000;//длительность интервала между продувками инкубатора от СО2
}
void PreSetTime()
...
label_1:
  ...
  if (buttons_Limit_Swith == 1)
  {
    ...
    if (Rotate == 1) {
      lcd.setCursor(0, 1);  lcd.print("start rotate EGG");  motor.turn_right(MotorSpeed, MoveTime);
    }
    else if (Rotate == 0) {
      lcd.setCursor(0, 1);  lcd.print("start rotate EGG");  motor.turn_left(MotorSpeed, MoveTime);
    }goto label_3;
  }
label_2:
  ...
  {
  label_3:
    lcd.setCursor(0, 1);  lcd.print("stop rotate EGG ");  motor.full_stop(100);
...
void Lite()
{
   if (Dey >= IncreaseDayHumiditiIncubation )digitalWrite(PinLite, LOW);
   if (Dey <= IncreaseDayHumiditiIncubation )digitalWrite(PinHot, HIGH);
...
void StartHum()
{ 
  if(Dey == IncreaseDayHumiditiIncubation && IDHI == 0)HumiditiIncubation=HumiditiIncubation*KHumiditiIncubation/100;IDHI=1;DDHI=0;
  if(Dey == DecreaseDayHumiditiIncubation && DDHI == 0)HumiditiIncubation=HumiditiIncubation*KHumiditiIncubation/100;IDHI=0;DDHI=1;
...
void TempRead()                                                     // Чтение температуры с датчика DS18b20
{
  if (currentMillis - ds18b20readMillis > ds18b20interval)
  {
    ...
    delay(100);                                    // Задержка перед поторным измерением
    sensors.requestTemperatures();                 // Запрос на измерение температуры (повторный)
    Tnow = float(sensors.getTempCByIndex(0));   // Получаем значение температуры
...
{
  PressingButtons = 0;
  buttons_Menu = analogRead(PinButtons);delay(50);
  int buttons_Menu1 = analogRead(PinButtons);delay(50);
  int buttons_Menu2 = analogRead(PinButtons);delay(50);
...
void loop()
{
  ...
  if (currentTime >= (loopTime))
  {
            ...
            if (MainMenu == 5 && SubMenu != 0 && FlagMenu == 0) { SubMenu--;  StartMillis = currentMillis;  delay(200);  if (SubMenu < 1)  SubMenu = 5; }
              ...
              if (MainMenu == 6 && SubMenu != 0 && FlagMenu == 0) { SubMenu--;  StartMillis = currentMillis;  delay(200);  if (SubMenu < 1)  SubMenu = 3; }
                ...
                if (MainMenu == 1 && FlagMenu == 1) {
                  ...
                  case 5: { DecreaseDayHumiditiIncubation = DecreaseDayHumiditiIncubation + 1; StartMillis = currentMillis; if (DecreaseDayHumiditiIncubation >= MaximumDecreaseDayHumiditiIncubation)DecreaseDayHumiditiIncubation = MaximumDecreaseDayHumiditiIncubation;           delay(200);  break; }
                  case 6: { IncreaseDayHumiditiIncubation = IncreaseDayHumiditiIncubation + 1; StartMillis = currentMillis; if (IncreaseDayHumiditiIncubation >= MaximumIncreaseDayHumiditiIncubation)IncreaseDayHumiditiIncubation = MaximumIncreaseDayHumiditiIncubation;           delay(200);  break; }
                  case 7: { KHumiditiIncubation = KHumiditiIncubation + 1; StartMillis = currentMillis; if (KHumiditiIncubation >= MaximumKHumiditiIncubation)KHumiditiIncubation = MaximumKHumiditiIncubation;           delay(200);  break; }
                  case 8: { i++;  if (i > 8)  i = 0;  TimeRotations = RotationPerDay[i];         StartMillis = currentMillis;                                                                                                     delay(200);  break; }
                  case 9: { maxTempFanStart = maxTempFanStart + FadeAmountTemp;                    StartMillis = currentMillis; if (maxTempFanStart >= maxTempFanStartMenuMax)maxTempFanStart = maxTempFanStartMenuMax;        delay(200);  break; }
                  case 10: { TimeFanWork = TimeFanWork + FadeTimeFanWork; StartMillis = currentMillis; if (TimeFanWork >= maxTimeFanWork) TimeFanWork = maxTimeFanWork; TimeFaningInterval = TimeFanWork * 1000;  delay(200);  break; }
                  case 11: { language = language + 1; StartMillis = currentMillis; if (language >= 3)KHumiditiIncubation = 3;           delay(200);  break; }
                  case 12: { TimeIntervalFanWork = TimeIntervalFanWork + FadeTimeIntervalFanWork; StartMillis = currentMillis; if (TimeIntervalFanWork >= maxTimeIntervalFanWork)TimeIntervalFanWork = maxTimeIntervalFanWork; TimeIntervalFaningInterval = TimeIntervalFanWork * 60000; delay(200);  break; }
                  case 13: { StartMillis = currentMillis; if (FanWorkFlag == 0) FanWorkFlag = 1; else if (FanWorkFlag == 1) FanWorkFlag = 0; delay(100); break; }
                  }
                }
                else {
                    ...
                    if (MainMenu == 3 && FlagMenu == 0 && SubMenu == 1) { bank++; StartMillis = currentMillis;  delay(200); if (bank > 4) bank = 4; }
                      ...
                      if (MainMenu == 4 && FlagMenu == 1) {
                        ...
                        if (MainMenu == 5 && FlagMenu == 1) {
                          switch (SubMenu) {
                          case 1: {  k++; if (k > 3) k = 3; PinHot = OutputPin[k]; StartMillis = currentMillis; delay(200); EEPROM.write(100, PinHot); delay(100); break; }
                          case 2: {  k++; if (k > 3) k = 3; PinFan = OutputPin[k]; StartMillis = currentMillis; delay(200); EEPROM.write(101, PinFan); delay(100); break; }
                          case 3: {  k++; if (k > 3) k = 3; PinHum = OutputPin[k]; StartMillis = currentMillis; delay(200); EEPROM.write(104, PinHum); delay(100); break; }}
                        }
                        else {
            ...
            if (MainMenu == 5 && SubMenu != 0 && FlagMenu == 0) { SubMenu++;  StartMillis = currentMillis;  delay(200);  if (SubMenu > 6)  SubMenu = 1; }
              ...
              if (MainMenu == 6 && SubMenu != 0 && FlagMenu == 0) { SubMenu++;  StartMillis = currentMillis;  delay(200);  if (SubMenu > 3)  SubMenu = 1; }
                ...
                if (MainMenu == 1 && FlagMenu == 1) {
                  ...
                  case 5: { DecreaseDayHumiditiIncubation = DecreaseDayHumiditiIncubation - 1; StartMillis = currentMillis; if (DecreaseDayHumiditiIncubation <= MinimumDecreaseDayHumiditiIncubation)DecreaseDayHumiditiIncubation = MinimumDecreaseDayHumiditiIncubation;           delay(200);  break; }
                  case 6: { IncreaseDayHumiditiIncubation = IncreaseDayHumiditiIncubation - 1; StartMillis = currentMillis; if (IncreaseDayHumiditiIncubation <= MinimumIncreaseDayHumiditiIncubation)IncreaseDayHumiditiIncubation = MinimumIncreaseDayHumiditiIncubation;           delay(200);  break; }
                  case 7: { KHumiditiIncubation = KHumiditiIncubation - 1; StartMillis = currentMillis; if (KHumiditiIncubation <= MinimumKHumiditiIncubation)KHumiditiIncubation = MinimumKHumiditiIncubation;           delay(200);  break; }
                  case 8: {  i--;  if (i < 0)  i = 8;  TimeRotations = RotationPerDay[i];            StartMillis = currentMillis;                                                                                                           delay(200);  break; }
                  case 9: {  maxTempFanStart = maxTempFanStart - FadeAmountTemp;                      StartMillis = currentMillis;  if (maxTempFanStart <= maxTempFanStartMenuMin)  maxTempFanStart = maxTempFanStartMenuMin;        delay(200);  break; }
                  case 10: {  TimeFanWork = TimeFanWork - FadeTimeFanWork;                             StartMillis = currentMillis;  if (TimeFanWork <= minTimeFanWork)  TimeFanWork = minTimeFanWork; TimeFaningInterval = TimeFanWork * 1000; delay(200);  break; }
                  case 11: { language = language - 1; StartMillis = currentMillis; if (language <= 1)KHumiditiIncubation = 1;           delay(200);  break; }
                  case 12: {  TimeIntervalFanWork = TimeIntervalFanWork - FadeTimeIntervalFanWork;     StartMillis = currentMillis;  if (TimeIntervalFanWork <= minTimeIntervalFanWork)  TimeIntervalFanWork = minTimeIntervalFanWork;  TimeIntervalFaningInterval = TimeIntervalFanWork * 60000;   delay(200);  break; }
                  case 13: { StartMillis = currentMillis; if (FanWorkFlag == 0) FanWorkFlag = 1; else if (FanWorkFlag == 1) FanWorkFlag = 0; delay(100); break; }
                  }
                }
                else {
                    ...
                    if (MainMenu == 3 && FlagMenu == 0 && SubMenu == 1) { bank--; StartMillis = currentMillis;  delay(200); if (bank < 0) bank = 0; }
                      ...
                      if (MainMenu == 4 && FlagMenu == 1) {
                        ...
                        if (MainMenu == 5 && FlagMenu == 1) {
                          ...
                          case 3: {  k--; if (k < 0) k = 0; PinHum = OutputPin[k]; StartMillis = currentMillis; delay(200); EEPROM.write(104, PinHum); delay(100); break; }
    ...
    case 5: {                                                                                                        // обработка события нажатия кнопки "ПЕРЕВОРОТ"
      Rotating();
      break; }
     case 6: {                                                                                                        // обработка события нажатия кнопки "Свет"
      Lite();
      break; }
    }
    switch (MainMenu) {
    case 0:      // главное меню "0"
      ...
      case 1:  m = 1; break;
      } // вывод на экран главного меню "0" подменю "1"
      break;
    ...
    case 2:      // главное меню "2"
      switch (SubMenu) {
      case 0:  m = 24; break;  // вывод на экран главного меню "2" подменю "0"
      case 1:  m = 25; break;
  ...
  //Serial.print("chek ");
  switch (m) {
  case 0: {  TempRead();  TempHumRead_DHT22();lcd.clear();lcd.setCursor(0, 0);  lcd.print("T1="); lcd.print(Tnow); lcd.print("\3 H="); lcd.print(hum); lcd.print("% ");  lcd.setCursor(0, 1);  lcd.print("T2="); lcd.print(temp); lcd.print("\3"); StartFan();  StartHot();  StartHum(); timerot();Lite();FlagMenu = 0;             break; }
  case 1: {  TempRead(); TempHumRead_DHT22();  RTC.getTime();  StartFan();  StartHot();  StartHum();  timerot();  TimePrint();Lite();                                                                                                                                                             FlagMenu = 0;             break; }
  ...
  case 19: {  lcd.clear(); lcd.setCursor(0, 0); lcd.print(F("   Cooling T    ")); lcd.setCursor(0, 1); lcd.print(F("t = ")); lcd.print(maxTempFanStart);        lcd.print("\3                ");                                      PrintMenuWrite(FlagMenu); delay(100);                           break; }
  case 20: {  lcd.clear(); lcd.setCursor(0, 0); lcd.print(F(" CO2 faning time")); lcd.setCursor(0, 1); lcd.print(F("time = ")); lcd.print(TimeFanWork);            lcd.print(" sec. ");                                      PrintMenuWrite(FlagMenu); delay(100);                           break; }
  case 21: {  lcd.clear(); lcd.setCursor(0, 0); lcd.print(F("   Language   ")); lcd.setCursor(0, 1); lcd.print(F(" ")); lcd.print(language);            lcd.print("  ");                                      PrintMenuWrite(FlagMenu); delay(100);                           break; }
  case 22: {  lcd.clear(); lcd.setCursor(0, 0); lcd.print(F("CO2 fan interval")); lcd.setCursor(0, 1); lcd.print(F("time = ")); lcd.print(TimeIntervalFanWork);    lcd.print(" min. ");                                      PrintMenuWrite(FlagMenu); delay(100);                           break; }
  case 23: {  lcd.clear(); lcd.setCursor(0, 0); lcd.print(F("   CO2 faning   ")); lcd.setCursor(0, 1); if (FanWorkFlag == 0) lcd.print("       OFF      "); else if (FanWorkFlag == 1) lcd.print("     IS ON      ");                       PrintMenuWrite(FlagMenu); delay(100);                           break; }
  case 24: {  lcd.clear(); lcd.setCursor(0, 0); lcd.print(F("  Save setting  ")); lcd.setCursor(0, 1); lcd.print(F("   to  EEPROM   ")); lcd.setCursor(15, 1);                lcd.print("\1");                                                                                  delay(100); FlagMenu = 0;             break; }
  case 25: {  lcd.clear(); lcd.setCursor(0, 0); lcd.print(F("Save setting to ")); lcd.setCursor(0, 1); lcd.print(F("bank ")); lcd.print(bank); if (FlagMenu == 0) { lcd.print(" press set"); delay(100); } else { SaveToEEPROM(bank); lcd.print(" saving...");      delay(100); }                          break; }
  ...
  case 51: {  lcd.clear(); lcd.setCursor(0, 0); lcd.print(F(" hotting  swith ")); lcd.setCursor(0, 1); lcd.print("in relay No="); int DispRelay = PinHot - 1;      if (DispRelay < 1) lcd.print("OFF"); else  lcd.print(DispRelay);  PrintMenuWrite(FlagMenu); delay(100);                           break; }
  ...
  case 53: {  lcd.clear(); lcd.setCursor(0, 0); lcd.print(F(" humiditi swith ")); lcd.setCursor(0, 1); lcd.print("in relay No="); int DispRelay = PinHum - 1;       if (DispRelay < 1) lcd.print("OFF"); else  lcd.print(DispRelay);  PrintMenuWrite(FlagMenu); delay(100);                           break; }
