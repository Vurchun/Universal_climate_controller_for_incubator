...
double temp;                                                          // Реальная температура на данный момент в инкубаторе на DHT22
...
void setup()
{
 ...
 myPID.SetMode(AUTOMATIC);
  ...
  consKp = EEPROM.read(17); delay(100);                                       
  consKi = EEPROM.read(18); delay(100);                                       
  consKd = EEPROM.read(19); delay(100);                                      
  ...
  lcd.setCursor(0, 1);  lcd.print("By_Vurchun_V-0.9");
  delay(3000);
  lcd.clear();
  dht.begin();
  currentTime = millis();
...
void SaveToEEPROM(int BankSave)                                                         // запись данных во внутренний EEPROM 
{
  ...
  BS = BankSave * 20 + 17;  EEPROM.write(BS,consKp); delay(100);  // запись Языка в ячейку "16" банка "bank"
  BS = BankSave * 20 + 18;  EEPROM.write(BS,consKi); delay(100);  // запись Языка в ячейку "16" банка "bank"
  BS = BankSave * 20 + 19;  EEPROM.write(BS,consKd); delay(100);  // запись Языка в ячейку "16" банка "bank"
}
void LoadFromEEPROM(int BankLoad)                                                       // загрузка данных из внутреннего EEPROM
{
  ...
  BL = BankLoad * 20 + 17; consKp = EEPROM.read(BL); delay(100);                                       
  BL = BankLoad * 20 + 18; consKi = EEPROM.read(BL); delay(100);                                       
  BL = BankLoad * 20 + 19; consKd = EEPROM.read(BL); delay(100);  
  TimeFaningInterval = TimeFanWork * 1000;                 //длительность работы вентилятора при продувке инкубатора от СО2 
  TimeIntervalFaningInterval = TimeIntervalFanWork * 60000;//длительность интервала между продувками инкубатора от СО2
}
void PreSetTime()
...
void loop()
{
  ...
  //Serial.print("chek ");
  switch (m) {
  case 0: {  TempRead();  TempHumRead_DHT22();lcd.clear();lcd.setCursor(0, 0);  lcd.print("T1="); lcd.print(Tnow); lcd.print("\3 H="); lcd.print(hum); lcd.print("% ");  lcd.setCursor(0, 1);  lcd.print("T2="); lcd.print(temp); lcd.print("\3"); StartFan();  StartHot();  StartHum(); timerot();FlagMenu = 0;             break; }
